...
import type { EventFilters, MonthOption } from '~/composables/useEvents'
...
defineProps<{ countries: string[]; regions: string[]; monthOptions: MonthOption[] }>()
</script>

<template>
  <div class="space-y-3 rounded-xl border border-stone-800 bg-stone-950 p-4 shadow-sm">
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      ...
      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-cream-300/50">State / Region</label>
        <select
      ...
      <div>
        <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-cream-300/50">Jump to month</label>
        <select
          v-model="month"
          class="w-full rounded-md border border-stone-700 bg-black px-3 py-2 text-sm text-cream-100 focus:border-moss-500 focus:outline-none focus:ring-1 focus:ring-moss-500"
        >
